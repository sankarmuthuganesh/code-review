 ...
 * @author optimus team
 */
public class OptimusJavaScriptCodeReview {
   ...
   private final Set<String> duplicateStringCheck = new HashSet<>(); // set to check the
   // duplicate
   // string

   private final List<String> prohibitedObjectList = Arrays.asList("window", "document", "screen", "history", "navigator", "location", "console");

   private final List<String> builtInObjectList = Arrays.asList("String", "Number", "Operators", "Statements", "Math", "Date", "Array", "Boolean",
      "Error", "RegExp", "Global", "Conversion", "Window", "Navigator", "Screen", "History", "Object", "Location", "Math");
   ...
   public void findBugs(File file) throws FileNotFoundException, IOException {
      ...
      while (tokenizer.ttype != StreamTokenizer.TT_EOF) {
         ...
         if (tokenizer.sval != null) {
            if (tokenizer.sval.equals("goog.provide")) {
               ...
               this.errorList.add("Line No: " + tokenizer.lineno() + " Use of function 'goog.net.XhrIo' is restricted." + "|"
               ...
               this.errorList.add("Line No: " + tokenizer.lineno() + " Use of function 'innerHTML' and 'innerText' is restricted." + "|"
                  + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
            // newly added rule for finding deprecated.
            // start
            else if ((tokenizer.sval.startsWith("goog.string.isEmpty") && tokenizer.sval.equals("goog.string.isEmpty"))) {
               this.errorList.add("Line No: " + tokenizer.lineno()
            ...
            else if (tokenizer.sval.startsWith("goog.string.isEmptySafe") && tokenizer.sval.equals("goog.string.isEmptySafe")) {
               this.errorList.add("Line No: " + tokenizer.lineno()
                  + " Use of deprecated 'goog.string.isEmptySafe' found.Please check https://google.github.io/closure-library/api/goog.string.html"
                  + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
            else if (tokenizer.sval.startsWith("goog.dom.getPageScroll") && tokenizer.sval.equals("goog.dom.getPageScroll")) {
               this.errorList.add("Line No: " + tokenizer.lineno()
               ...
               this.warningList.add("Line No: " + tokenizer.lineno() + " Use of deprecated 'goog.global' found.Please check greeneyeticket 211453"
                  + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
               ...
               this.warningList.add("Line No: " + tokenizer.lineno()
            ...
            else if (tokenizer.sval.startsWith("goog.dom.htmlToDocumentFragment") && tokenizer.sval.equals("goog.dom.htmlToDocumentFragment")) {
               this.warningList.add("Line No: " + tokenizer.lineno() + " Use of deprecated 'goog.dom.htmlToDocumentFragment'" + "|"
                  + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
            // end
            ...
            for (int i = 0; i < tokenList.size(); i++) {
               ...
               if (token.endsWith("_") && i > 0) {
                  ...
                  if ((tokenList.get(i - 1).equals("this")) || (tokenList.get(i - 1).equals("prototype")) || (token.equals(token.toUpperCase()))) {
                     // do nothing
                  }
                  else {
                     this.warningList.add("Line No: " + tokenizer.lineno() + " Access of private member " + token
            ...
            // this.warningList.add("Line No: "+tokenizer.lineno()+" Access of private
            // member found.(if it's not a
            // member of this js class,then should not use that.)");
            //
            // }

            prohibitedObjectList.forEach(object -> {
               if (tokenizer.sval.startsWith(object + ".") || tokenizer.sval.equals(object)) {
                  this.warningList.add("Line No: " + tokenizer.lineno() + " Use of browser object '" + tokenizer.sval
                     + "' found(use goog API instead)." + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
               }
            });
            builtInObjectList.forEach(builtInObject -> {
               if (tokenizer.sval.startsWith(builtInObject + ".prototype") || tokenizer.sval.equals(builtInObject)) {
                  this.warningList.add("Line No: " + tokenizer.lineno() + " Use of browser object or built-in type's prototype modification '"
                     + tokenizer.sval + "' is restricted." + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
               }
            });
         }
      }
      if (!isGoogScope) {
         this.warningList.add("Warning: goog.scope is not found." + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
      }
      ...
      visitor.scriptMethods.forEach(method -> {
         if (method.doc != null && !method.doc.sourceCode.startsWith(method.name.substring(method.name.lastIndexOf('.') + 1))) {
            this.errorList
               .add("Line No: " + method.startLine + " method doc should starts with method name." + "|" + BugCategory.DOCS_ISSUE.toString());
         }
         if (method.isConstructor) {
            AtomicBoolean isExtends = new AtomicBoolean(false);
            if (method.doc != null && !method.doc.annotations.stream().anyMatch(anno -> anno.name.equals("@constructor"))) {
               this.errorList.add("Line No: " + method.startLine + " @constructor doc is missing." + "|" + BugCategory.DOCS_ISSUE.toString());
            }

            if (method.doc != null && !method.doc.annotations.stream().anyMatch(anno -> {
               if (anno.name.equals("@extends") && anno.values.size() > 0 && anno.values.get(0).length() > 2 && anno.values.get(0).startsWith("{")
                  && anno.values.get(0).endsWith("}")) {
                  isExtends.set(true);
               }
               return anno.name.equals("@extends");
            })) {
               this.errorList.add("Line No: " + method.startLine + " @extends doc is missing." + "|" + BugCategory.DOCS_ISSUE.toString());
            }
            else if (!isExtends.get()) {
               this.errorList.add(
                  "Line No: " + method.startLine + " @extends doc should have inherited class as type." + "|" + BugCategory.DOCS_ISSUE.toString());
            }
         }
         if (method.isPrivate && !method.name.endsWith("_")) {
            ...
            this.errorList.add("Line No: " + method.startLine + " @private doc is missing for method '" + method.name + "'" + "|"
         ...
         if (method.doc != null && method.doc.annotations.stream().anyMatch(anno -> {
            ...
            this.errorList.add(
               "Line No: " + method.startLine + " unnecessary @return for method '" + method.name + "'" + "|" + BugCategory.DOCS_ISSUE.toString());
         }
         else if (method.isReturns && method.doc != null && !method.doc.annotations.stream().anyMatch(anno -> {
            return anno.name.equals("@return");
         })) {
            this.errorList.add(
               "Line No: " + method.startLine + " @return doc is missing for method '" + method.name + "'" + "|" + BugCategory.DOCS_ISSUE.toString());
         }
         else if (method.isReturns && method.doc != null && method.doc.annotations.stream().anyMatch(anno -> {
            return anno.name.equals("@return");
         })) {
            if (method.doc != null) {
               ScriptAnnotation annotation = method.doc.annotations.stream().filter(anno -> anno.name.equals("@return")).findFirst().get();
               if (!(annotation.values.size() > 1 && annotation.values.get(0).length() > 2 && annotation.values.get(0).startsWith("{")
                  && annotation.values.get(0).endsWith("}"))) {
                  this.errorList.add("Line No: " + method.startLine + " Type is missing for @return doc in method '" + method.name + "'" + "|"
                     + BugCategory.DOCS_ISSUE.toString());
               }
            }
         }
         method.paramList.forEach(param -> {
            ...
            if (method.doc != null && !method.doc.annotations.stream().anyMatch(anno -> {
               if (anno.values.size() > 1 && anno.values.get(0).length() > 2 && anno.values.get(0).startsWith("{")
                  && anno.values.get(0).endsWith("}")) {
                  isTypePresent.set(true);
               }
               return anno.values.contains(param.sourceCode);
            })) {
               this.errorList.add("Line No: " + param.lineNumber + " @param is missing for param '" + param.sourceCode + "'" + "|"
                  + BugCategory.DOCS_ISSUE.toString());
            }
            if (!isTypePresent.get()) {
               this.errorList.add(
                  "Line No: " + param.lineNumber + " type is missing for param '" + param.sourceCode + "'" + "|" + BugCategory.DOCS_ISSUE.toString());
            }
         });
      });
   ...
   private void validateFile() throws IOException {
      ...
      if (!name.matches("[a-z\\-]+[a-z]+")) {
         this.errorList.add("Error: file name is wrong." + "|" + BugCategory.FILE_NAMING_ISSUE.toString());
      }
   }
   ...
   private class JSVoidVisitor implements NodeVisitor {
      ...
      List<String> prohibitedCalls = Arrays.asList("eval", "with", "alert", "setTimeout", "setInterval", "goog.dom.get", "goog.dom.classes.",
         "goog.style.", "goog.events.listen", "parseInt", "parseFloat", "blur", "decodeURI", "encodeURI", "keys");
      ...
      private boolean hasReturnStatement(FunctionNode node) {
         ...
         class ReturnVisitor implements NodeVisitor {
            ...
            public boolean visit(AstNode visitedNode) {
               if (visitedNode.getType() == Token.RETURN && visitedNode.getEnclosingFunction().equals(node)) {
      ...
      public boolean visit(AstNode node) {
         if (node.getType() == Token.ASSIGN) {
            ...
            if (assignment.getRight() instanceof Assignment) {
               OptimusJavaScriptCodeReview.this.errorList.add("Line No: " + assignment.getLineno() + " Don't summarize variables together." + "|"
                  + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
            if (assignment.getRight().getType() == Token.FUNCTION) {
               FunctionNode function = (FunctionNode) assignment.getRight();
               if (assignment.getLeft().toSource().trim().equals(fileProvide) || assignment.getLeft().toSource().trim().contains(".prototype.")) {
                  ...
                  if (assignment.getJsDocNode() != null) {
                     ...
                     OptimusJavaScriptCodeReview.this.errorList.add("Line No: " + assignment.getLineno() + " Doc is missing for method '"
                        + method.name + "'" + "|" + BugCategory.DOCS_ISSUE.toString());
                  }
               ...
               if (assignment.getRight().toSource().contains("wap.ri.core.InputFormModelHelper")) {
                  OptimusJavaScriptCodeReview.this.errorList.add("Line No: " + assignment.getLineno()
            ...
            if (variableList.size() > 1) {
               OptimusJavaScriptCodeReview.this.errorList.add(
                  "Line No: " + varDec.getLineno() + " Don't declare variables together." + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
            else if (variableList.size() == 1) {
               if (!variableList.get(0).getTarget().toSource().matches("(\\$)?[a-z]+[A-Za-z_]*")) {
                  OptimusJavaScriptCodeReview.this.warningList.add("Line No: " + varDec.getLineno() + " Variable name should be in lower camel case '"
                     + variableList.get(0).getTarget().toSource() + "'" + "|" + BugCategory.VARIABLE_METHOD_NAMING_ISSUE.toString());
               }
               if (variableList.get(0).getTarget().toSource().length() < 3) {
                  OptimusJavaScriptCodeReview.this.errorList.add("Line No: " + varDec.getLineno() + " Variable name should be meaningful. '"
                     + variableList.get(0).getTarget().toSource() + "'" + "|" + BugCategory.VARIABLE_METHOD_NAMING_ISSUE.toString());
               }
            }
         }
            ...
            objLiteral.getElements().forEach(objectProperty -> {
               if (objectProperty.getLeft() instanceof StringLiteral) {
                  ...
                  // "+objLiteral.getLineno()+" Don't use
                  // string literal as key in object.");
               }
            });
         }
            ...
            OptimusJavaScriptCodeReview.this.errorList.add(
               "Line No: " + expr.getLineno() + " Don't use '" + expr.toSource() + "'" + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
         }
         else if (node.getType() == Token.SHNE || node.getType() == Token.SHEQ) {
            InfixExpression expr = (InfixExpression) node;
            if (expr.getRight().toSource().equals("null") || expr.getRight().toSource().equals("undefined")) {
               OptimusJavaScriptCodeReview.this.errorList.add("Line No: " + expr.getLineno() + " Don't check '" + expr.toSource()
                  + "'.Instead use (!variable)or(variable)" + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
            else if (expr.getRight().toSource().equals("true") || expr.getRight().toSource().equals("false")) {
               OptimusJavaScriptCodeReview.this.errorList.add("Line No: " + expr.getLineno() + " Don't do unnecessary boolean comparisions. '"
                  + expr.toSource() + "'" + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
         }
         else if (node.getType() == Token.CALL) {
            prohibitedCalls.forEach(call -> {
               if (node.toSource().startsWith(call)
                  && !(node.toSource().startsWith("goog.events.listenOnce") || node.toSource().startsWith("goog.events.listenWith"))) {
                  if (!node.toSource().startsWith("goog.dom.getWindow")) {
                     OptimusJavaScriptCodeReview.this.warningList.add("Line No: " + node.getLineno() + " Use of prohibited method call '"
                        + node.toSource() + "'" + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
                  }
               }
            });
         }
         else if (node.getType() == Token.NUMBER || node.getType() == Token.STRING) {
            AstNode parent = node.getParent();
            if (node.toSource().startsWith("\"") && node.toSource().endsWith("\"")) {
               OptimusJavaScriptCodeReview.this.errorList.add("Line No: " + node.getLineno() + " Single quote is prefered over double quote." + "|"
                  + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
            if (node.toSource().equalsIgnoreCase("'use strict'")
               && (node.getParent().getParent().getType() == Token.BLOCK || node.getParent().getParent().getType() == Token.SCRIPT)) {
               OptimusJavaScriptCodeReview.this.warningList
                  .add("Line No: " + node.getLineno() + " Don't use 'use strict'" + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
            if (!(parent != null && parent.getParent().getType() == Token.OBJECTLIT)) {
               if (!node.getParent().toSource().trim().startsWith("goog.") && !node.getParent().toSource().trim().startsWith("goog.require")
                  ...
                  if (!duplicateStringCheck.add(node.toSource())) {
                     OptimusJavaScriptCodeReview.this.warningList.add(
                        "Line No: " + node.getLineno() + " Don't hard code literal" + node.toSource() + "|" + BugCategory.HARDCODE_ISSUE.toString());
                  }
                  // end
               }
            ...
            if (node.getEnclosingScope().getType() == Token.SCRIPT) {
               OptimusJavaScriptCodeReview.this.errorList.add(
                  "Line No: " + node.getLineno() + " Global variable is not allowed." + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
         }
         else if (node instanceof ForInLoop) {
            OptimusJavaScriptCodeReview.this.errorList
               .add("Line No: " + node.getLineno() + " Don't use for-in loop." + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
         }
         else if (node.getType() == Token.COMMENT) {
            Comment jsComment = (Comment) node;
            if (jsComment.getCommentType() == CommentType.BLOCK_COMMENT || jsComment.getCommentType() == CommentType.LINE) {
               OptimusJavaScriptCodeReview.this.warningList
                  .add("Line No: " + jsComment.getLineno() + " commented lines found." + "|" + BugCategory.CONVENTION_VIOLATIONS_ISSUE.toString());
            }
         }
         return true;
      }
